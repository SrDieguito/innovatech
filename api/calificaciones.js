...
import { getCookieMap } from './_utils/cookies.js';
...
async function isOwnerOrAdmin(userId, cursoId) {
  ...
  try {
    const [rows] = await pool.query(
      'SELECT 1 FROM cursos WHERE id = ? AND profesor_id = ? LIMIT 1',
      [cursoId, userId]
    );
    if (rows.length > 0) return true;
    const [roles] = await pool.query('SELECT rol FROM usuarios WHERE id = ? LIMIT 1', [userId]);
    return roles.length > 0 && roles[0].rol === 'admin';
  } catch { return false; }
}
...
export default async function handler(req, res) {
  try {
    ...
    if (action !== 'listar_por_curso') return json(res, 400, { error: 'Acción inválida' });
    ...
    const puedeVerTodo = await isOwnerOrAdmin(userId, curso_id);
    ...
    if (!puedeVerTodo && role === 'estudiante') {
      ...
      if (ins.length === 0) return json(res, 403, { error: 'No autorizado' });
    }

    const [rows] = await pool.query(
      `SELECT
         t.id AS tarea_id, t.titulo AS tarea_titulo, t.fecha_limite AS tarea_fecha_limite,
         u.id AS estudiante_id, u.email AS estudiante_nombre,
         te.id AS entrega_id, te.calificacion, te.observacion AS observaciones, te.fecha_entrega
       FROM tareas t
       INNER JOIN cursos c ON c.id = t.curso_id
       INNER JOIN cursos_estudiantes ce ON ce.curso_id = c.id
       INNER JOIN usuarios u ON u.id = ce.estudiante_id
       LEFT JOIN tareas_entregas te ON te.tarea_id = t.id AND te.estudiante_id = u.id
       WHERE t.curso_id = ?
    ...
    for (const r of rows) {
      if (!puedeVerTodo && role === 'estudiante' && r.estudiante_id !== userId) continue;
    ...
    const tareas = Array.from(tareasMap.values()).map((t) => {
      ...
      return { ...t, promedio, evaluadas: notas.length, total: t.estudiantes.length, pendientes: t.estudiantes.length - notas.length };
    });

    return json(res, 200, { curso_id, puedeVerTodo, tareas });
  } catch (err) {
    console.error('GET /api/calificaciones error:', err);
    return json(res, 500, { error: 'INTERNAL_SERVER_ERROR', message: err?.message || 'Error desconocido' });
  }
}
